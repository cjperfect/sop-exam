...
import type { QuestionType } from "@sop/shared";
        ...
        {question.type === "single_choice" && question.options && (
          ...
          >
            {question.options.map((opt) => (
              <div
                key={opt.key}
                className="flex items-center gap-3 rounded-lg border p-3 transition-colors has-[:checked]:border-primary has-[:checked]:bg-primary/5"
              >
                <RadioGroupItem
                  value={opt.key}
                  id={`q-${question.sortOrder}-${opt.key}`}
                />
                <Label
                  htmlFor={`q-${question.sortOrder}-${opt.key}`}
