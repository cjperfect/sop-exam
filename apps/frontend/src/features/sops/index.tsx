...
export function SopList() {
  ...
  const [view, setView] = useState<ViewMode>(() => (localStorage.getItem("sop_view") as ViewMode) || "card");
  ...
  const { data: sopsRes, isLoading } = useQuery({
    queryKey: ["sops", page],
    queryFn: () => fetchSops({ page, pageSize: 10 }),
  });
  const sops = sopsRes?.items ?? [];
  const total = sopsRes?.total ?? 0;

  const departments = useMemo(() => {
    return [...new Set(sops.map((s) => s.department))].sort();
  }, [sops]);
  ...
  const filtered = useMemo(() => {
    return sops.filter((sop) => {
      const matchSearch = !search || sop.title.toLowerCase().includes(search.toLowerCase());
      ...
      <Main>
        <div className="mb-4 flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground">浏览和学习企业标准作业程序文档</p>
          </div>
          <ViewToggle value={view} onChange={handleViewChange} />
        </div>

        <div className="mb-6">
          <SopDepartmentFilter selected={department} onSelect={setDepartment} departments={departments} />
        </div>
        ...
        {!isLoading && total > 10 && <Pagination page={page} pageSize={10} total={total} onPageChange={setPage} />}
      </Main>
    </>
  );
}
